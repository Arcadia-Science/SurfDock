...
def _build_cache_path(
    ...
    if all_atoms:
        full += f"_atomRad{atom_radius}_atomMax{atom_max_neighbors}"
    if matching and num_conformers != 1:
        full += f"_confs{num_conformers}"
    if esm_embeddings_path is not None:
        full += f"_{esm_model_name}"
    full += f"_pocket{pocket_cutoff}"
...
def _process_one_complex(
    ...
    rec_path = os.path.join(surface_path, name, f"{name}_protein_processed_{pocket_cutoff}.pdb")
    ...
    lig_path = os.path.join(data_dir, name, f"{name}_ligand.sdf")
    lig = read_abs_file_mol(lig_path, remove_hs=False, sanitize=True)
    if lig is None:
        return [], []

    if max_lig_size is not None and lig.GetNumHeavyAtoms() > max_lig_size:
        return [], []
    ...
    if (not matching) or num_conformers == 1:
    ...
    ply_path = os.path.join(surface_path, name, f"{name}_protein_processed_{pocket_cutoff}.ply")
        ...
        for name in names:
            ...
            if esm_embeddings_path is not None:
                emb_path = os.path.join(
                    esm_embeddings_path,
                    name,
                    f"{name}_protein_processed_{pocket_cutoff}_{esm_model_name}.pt",
                )
...
def _build_args(
    ...
    split_train = str(DATA_ROOT / "splits" / "timesplit_no_lig_overlap_train_surface_complete")
    split_val = str(DATA_ROOT / "splits" / "timesplit_no_lig_overlap_val_surface_complete")

    return Namespace(
        config=None,
        log_dir=str(DATA_ROOT / "workdir"),
        restart_dir=restart_dir or None,
        ...
        run_name=run_name or f"modal_{n_epochs}ep",
        use_ema=use_ema,
        ema_rate=ema_rate,
        ...
        atom_max_neighbors=atom_max_neighbors,
        matching=matching,
        matching_popsize=matching_popsize,
        matching_maxiter=matching_maxiter,
        num_conformers=num_conformers,
        ...
        skip_inference_freq=skip_inference_freq,
        num_inference_complexes=num_inference_complexes,
        ...
        topN=topN,
        mdn_early_stop_patience=mdn_early_stop_patience,
        mdn_dropout=mdn_dropout,
        n_gaussians=n_gaussians,
        ligand_distance_prediction=ligand_distance_prediction,
        atom_type_prediction=atom_type_prediction,
        bond_type_prediction=bond_type_prediction,
        residue_type_prediction=residue_type_prediction,
...
def train(
    ...
    args = _build_args(
        ...
        matching_popsize=matching_popsize, matching_maxiter=matching_maxiter,
        num_conformers=num_conformers, max_lig_size=max_lig_size,
        scale_by_sigma=scale_by_sigma, cudnn_benchmark=cudnn_benchmark,
        test_sigma_intervals=test_sigma_intervals,
    )
